...
{
    /// <inheritdoc />
    public partial class AddPolicyNumberAssignmentProgramScopeRefs : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(
                """
                UPDATE policy_number_assignments
                SET "State" = NULLIF(UPPER(TRIM("State")), '')
                WHERE "State" IS NOT NULL;

                UPDATE policy_number_assignments p
                ...
                BEGIN
                    IF EXISTS (
                        SELECT 1
                        FROM policy_number_assignments p
                        WHERE p."ProgramConfigurationId" IS NOT NULL
                          AND NOT EXISTS (
                              SELECT 1
                        ...
                        RAISE EXCEPTION 'Cannot add policy number assignment Program SOT constraint: at least one assignment references an inactive or deleted Program.';
                    END IF;
                    ...
                    IF EXISTS (
                        ...
                        RAISE EXCEPTION 'Cannot add policy number assignment Program SOT constraint: a Program/Carrier/LOB assignment has no matching active ProgramCarrierLineOfBusiness path.';
                    END IF;
                    ...
                    IF EXISTS (
                        ...
                        RAISE EXCEPTION 'Cannot add policy number assignment Program SOT constraint: a Program/Carrier/LOB/State assignment has no matching active ProgramCarrierLobState path.';
                    END IF;
                END $$;

                CREATE OR REPLACE FUNCTION validate_policy_number_assignment_program_scope()
                RETURNS trigger AS $$
                BEGIN
                    IF NEW."ProgramConfigurationId" IS NULL THEN
                        IF NEW."ProgramCarrierLineOfBusinessId" IS NOT NULL OR NEW."ProgramCarrierLobStateId" IS NOT NULL THEN
                            RAISE EXCEPTION 'Policy number assignment without ProgramConfigurationId cannot reference Program setup scope ids.';
                        END IF;
                    ...
                    IF NOT EXISTS (
                        ...
                        RAISE EXCEPTION 'Policy number assignment ProgramConfigurationId is not active.';
                    END IF;

                    IF NEW."State" IS NULL THEN
                        IF NEW."ProgramCarrierLineOfBusinessId" IS NULL OR NEW."ProgramCarrierLobStateId" IS NOT NULL THEN
                            RAISE EXCEPTION 'Program all-state policy number assignment requires ProgramCarrierLineOfBusinessId only.';
                        END IF;
                        ...
                        IF NOT EXISTS (
                            ...
                            RAISE EXCEPTION 'Policy number assignment ProgramCarrierLineOfBusinessId does not match Program, Carrier, and LineOfBusiness.';
                        END IF;
                    ...
                    IF NEW."ProgramCarrierLobStateId" IS NULL OR NEW."ProgramCarrierLineOfBusinessId" IS NOT NULL THEN
                        RAISE EXCEPTION 'Program state-specific policy number assignment requires ProgramCarrierLobStateId only.';
                    END IF;
                    ...
                    IF NOT EXISTS (
                        ...
                        RAISE EXCEPTION 'Policy number assignment ProgramCarrierLobStateId does not match Program, Carrier, LineOfBusiness, and State.';
                    END IF;
                ...
                CREATE TRIGGER trg_validate_policy_number_assignment_program_scope
                ...
                ON policy_number_assignments
                FOR EACH ROW
                EXECUTE FUNCTION validate_policy_number_assignment_program_scope();

                CREATE OR REPLACE FUNCTION validate_existing_policy_number_assignment_program_scopes()
                RETURNS trigger AS $$
                BEGIN
                    IF TG_TABLE_NAME = 'program_carriers' THEN
                        IF EXISTS (
                            ...
                            RAISE EXCEPTION 'Program setup change would invalidate existing policy number assignment ProgramCarrierLineOfBusinessId.';
                        END IF;
                        ...
                        IF EXISTS (
                            ...
                            RAISE EXCEPTION 'Program setup change would invalidate existing policy number assignment ProgramCarrierLobStateId.';
                        END IF;
                    END IF;
                ...
                CREATE TRIGGER trg_validate_policy_number_assignments_after_program_carrier_change
                AFTER UPDATE OF "ProgramConfigurationId", "CarrierId"
                ON program_carriers
                FOR EACH ROW
                EXECUTE FUNCTION validate_existing_policy_number_assignment_program_scopes();

                CREATE TRIGGER trg_validate_policy_number_assignments_after_program_lob_change
                AFTER UPDATE OF "ProgramCarrierId", "LineOfBusiness"
                ON program_carrier_lines_of_business
                FOR EACH ROW
                EXECUTE FUNCTION validate_existing_policy_number_assignment_program_scopes();

                CREATE TRIGGER trg_validate_policy_number_assignments_after_program_state_change
            ...
            migrationBuilder.CreateIndex(
                name: "ix_policy_number_assignment_program_lob_scope",
                table: "policy_number_assignments",
                column: "ProgramCarrierLineOfBusinessId");

            migrationBuilder.CreateIndex(
                name: "ix_policy_number_assignment_program_state_scope",
                table: "policy_number_assignments",
                column: "ProgramCarrierLobStateId");

            migrationBuilder.AddCheckConstraint(
                name: "ck_policy_number_assignment_program_scope_canonical",
                table: "policy_number_assignments",
            ...
            migrationBuilder.AddForeignKey(
                name: "FK_policy_number_assignments_program_carrier_lines_of_business~",
                table: "policy_number_assignments",
                column: "ProgramCarrierLineOfBusinessId",
            ...
            migrationBuilder.AddForeignKey(
                name: "FK_policy_number_assignments_program_carrier_lob_states_Progra~",
                table: "policy_number_assignments",
                column: "ProgramCarrierLobStateId",
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                """
                DROP TRIGGER IF EXISTS trg_validate_policy_number_assignments_after_program_state_change ON program_carrier_lob_states;
                DROP TRIGGER IF EXISTS trg_validate_policy_number_assignments_after_program_lob_change ON program_carrier_lines_of_business;
                DROP TRIGGER IF EXISTS trg_validate_policy_number_assignments_after_program_carrier_change ON program_carriers;
                DROP TRIGGER IF EXISTS trg_validate_policy_number_assignment_program_scope ON policy_number_assignments;
                DROP FUNCTION IF EXISTS validate_existing_policy_number_assignment_program_scopes();
                DROP FUNCTION IF EXISTS validate_policy_number_assignment_program_scope();
                """);
